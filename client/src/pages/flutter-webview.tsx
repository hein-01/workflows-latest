...
export default function FlutterWebView() {
  const [configs, setConfigs] = useState<string[]>(Array(50).fill(""));
  const [configLabels, setConfigLabels] = useState<string[]>(Array(50).fill(""));
  const [editingLabel, setEditingLabel] = useState<number | null>(null);
  const [tempLabel, setTempLabel] = useState("");
  const [configsCopied, setConfigsCopied] = useState<boolean[]>(Array(50).fill(false));
  const [configsSaving, setConfigsSaving] = useState<boolean[]>(Array(50).fill(false));
  const [configsSaved, setConfigsSaved] = useState<boolean[]>(Array(50).fill(false));
  ...
  const loadConfigs = async () => {
    ...
    if (data) {
      const loadedConfigs = Array(50).fill("");
      const loadedLabels = Array(50).fill("");
      data.forEach(config => {
        if (config.config_index >= 1 && config.config_index <= 50) {
          ...
          <CardHeader className="pb-4">
            <div className="flex items-center space-x-2">
              ...
              <CardTitle className="text-xl font-semibold text-gray-800">
                AI Toys Configuration
              </CardTitle>
            </div>
          </CardHeader>
          <CardContent className="space-y-6">
            {Array.from({ length: 50 }, (_, i) => i + 1).map((configNum) => {
