...
import { Copy, CheckCircle, FileText, Check, X, Sparkles } from "lucide-react";
...
export default function General() {
  ...
  // Vibe Coding state
  const [vibeConfigs, setVibeConfigs] = useState<string[]>(Array(30).fill(""));
  const [vibeConfigLabels, setVibeConfigLabels] = useState<string[]>(Array(30).fill(""));
  const [vibeEditingLabel, setVibeEditingLabel] = useState<number | null>(null);
  const [vibeTempLabel, setVibeTempLabel] = useState("");
  const [vibeConfigsCopied, setVibeConfigsCopied] = useState<boolean[]>(Array(30).fill(false));
  const [vibeConfigsSaving, setVibeConfigsSaving] = useState<boolean[]>(Array(30).fill(false));
  const [vibeConfigsSaved, setVibeConfigsSaved] = useState<boolean[]>(Array(30).fill(false));
  ...
  const loadVibeConfigs = async () => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;

    const { data, error } = await supabase
      .from('vibe_coding_configs')
      .select('*')
      .eq('user_id', user.id)
      .order('config_index');

    if (error) {
      console.error('Error loading vibe configs:', error);
      return;
    }
    ...
    if (data) {
      ...
      setVibeConfigs(loadedConfigs);
      setVibeConfigLabels(loadedLabels);
    }
  };
  ...
  const updateVibeConfigValue = (index: number, value: string) => {
    const newConfigs = [...vibeConfigs];
    newConfigs[index] = value;
    setVibeConfigs(newConfigs);
  };

  const setVibeConfigCopied = (index: number, copied: boolean) => {
    const newConfigsCopied = [...vibeConfigsCopied];
    newConfigsCopied[index] = copied;
    setVibeConfigsCopied(newConfigsCopied);
  };

  const startVibeEditingLabel = (index: number) => {
    setVibeEditingLabel(index);
    setVibeTempLabel(vibeConfigLabels[index] || `Config ${index + 1}`);
  };

  const cancelVibeEditingLabel = () => {
    setVibeEditingLabel(null);
    setVibeTempLabel("");
  };

  const saveVibeLabel = async (index: number) => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      toast({
        title: "Authentication required",
    ...
    const { error } = await supabase
      ...
      .upsert({
        user_id: user.id,
        config_index: index + 1,
        config_text: vibeConfigs[index] || "",
        config_label: vibeTempLabel.trim() || null,
      }, {
        onConflict: 'user_id,config_index'
      });

    if (error) {
      console.error('Error saving vibe label:', error);
      toast({
        title: "Error saving label",
        description: error.message,
        variant: "destructive",
    ...
    const newLabels = [...vibeConfigLabels];
    newLabels[index] = vibeTempLabel.trim();
    setVibeConfigLabels(newLabels);
    setVibeEditingLabel(null);
    setVibeTempLabel("");

    toast({
      title: "Label saved",
      description: "Vibe Coding label has been updated",
    });
  };

  const saveVibeConfig = async (index: number) => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      toast({
        title: "Authentication required",
    ...
    const configText = vibeConfigs[index];
    if (!configText.trim()) {
      toast({
        title: "Empty config",
        description: "Please enter some text before saving",
    ...
    const newConfigsSaving = [...vibeConfigsSaving];
    newConfigsSaving[index] = true;
    setVibeConfigsSaving(newConfigsSaving);
    ...
    const { error } = await supabase
      ...
      .upsert({
        ...
        config_index: index + 1,
        config_text: configText,
        config_label: vibeConfigLabels[index] || null,
      }, {
        onConflict: 'user_id,config_index'
      });

    const newConfigsSaving2 = [...vibeConfigsSaving];
    newConfigsSaving2[index] = false;
    setVibeConfigsSaving(newConfigsSaving2);

    if (error) {
      console.error('Error saving vibe config:', error);
      toast({
        title: "Error saving config",
        description: error.message,
        variant: "destructive",
    ...
    const newConfigsSaved = [...vibeConfigsSaved];
    newConfigsSaved[index] = true;
    setVibeConfigsSaved(newConfigsSaved);

    toast({
      title: "Saved successfully",
      description: `Vibe Coding ${index + 1} has been saved`,
    });

    setTimeout(() => {
      const resetSaved = [...newConfigsSaved];
      resetSaved[index] = false;
      setVibeConfigsSaved(resetSaved);
    }, 2000);
  };
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <Card className="mb-6 shadow-lg">
          ...
          <CardContent className="space-y-6">
            {Array.from({ length: 30 }, (_, i) => i + 1).map((configNum) => {
              const configIndex = configNum - 1;
              const configValue = vibeConfigs[configIndex];
              const configCopied = vibeConfigsCopied[configIndex];
              const configSaving = vibeConfigsSaving[configIndex];
              const configSaved = vibeConfigsSaved[configIndex];

              return (
                <div key={configNum}>
                  <div className="flex items-center gap-2 mb-1.5">
                    {vibeEditingLabel === configIndex ? (
                      <>
                        <Input
                          value={vibeTempLabel}
                          onChange={(e) => setVibeTempLabel(e.target.value)}
                          className="h-7 text-sm"
                          autoFocus
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') saveVibeLabel(configIndex);
                            if (e.key === 'Escape') cancelVibeEditingLabel();
                          }}
                        />
                        <Button
                          variant="default"
                          size="sm"
                          onClick={() => saveVibeLabel(configIndex)}
                          className="h-7 px-2 text-white"
                          style={{ backgroundColor: '#8041CE' }}
                          onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#6d35b8'}
                          onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#8041CE'}
                        ...
                        <Button
                          variant="secondary"
                          size="sm"
                          onClick={cancelVibeEditingLabel}
                          className="h-7 px-2"
                        >
                          <X className="h-3 w-3 mr-1" />
                          Cancel
                      ...
                      <>
                        <Label htmlFor={`vibe-config-${configNum}`} className="text-sm font-medium" style={{ color: '#8041CE' }}>
                          {vibeConfigLabels[configIndex] || `Config ${configNum}`}
                        </Label>
                        <Button
                          variant="secondary"
                          size="sm"
                          onClick={() => startVibeEditingLabel(configIndex)}
                          className="h-6 px-2 text-xs"
                        >
                          Edit
                        </Button>
                  ...
                  <div className="relative mt-1.5">
                    <Textarea
                      id={`vibe-config-${configNum}`}
                      placeholder={`Enter config ${configNum}`}
                      value={configValue}
                      onChange={(e) => updateVibeConfigValue(configIndex, e.target.value)}
                      className="min-h-[100px] pr-2 pb-12 resize-y"
                      rows={3}
                    />
                    <div className="absolute right-2 bottom-2 flex gap-1">
                      <Button
                        variant="default"
                        size="sm"
                        onClick={() => saveVibeConfig(configIndex)}
                        disabled={configSaving}
                        className="h-8 px-3 text-white"
                        style={{ backgroundColor: configSaving ? '#9f7aea' : '#8041CE' }}
                        onMouseEnter={(e) => !configSaving && (e.currentTarget.style.backgroundColor = '#6d35b8')}
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (configCopied) {
                            setVibeConfigCopied(configIndex, false);
                          } else {
                            navigator.clipboard.writeText(configValue);
                            setVibeConfigCopied(configIndex, true);
                          }
                        }}
                        className="h-8 px-3"
                      >
